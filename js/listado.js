var elementosFiltrados = function (afiltrar) {
    var filtrado = afiltrar.filter((elem, index, self) => index === self.indexOf(elem));
...
Listado.prototype.buscarRestaurante = function (id) {
    var restaurantes = this.restaurantes.find(function (elemento) {
...
Listado.prototype.obtenerUbicaciones = function () {
    ...
    var ciudades = this.restaurantes.map(ciudad => ciudad.ubicacion);
    return elementosFiltrados(ciudades);
}
...
Listado.prototype.obtenerRubros = function () {
    var rubros = this.restaurantes.map(valores => valores.rubro);

    return elementosFiltrados(rubros);
}
...
Listado.prototype.obtenerHorarios = function () {
    ...
    var arrayHorariosTemp = this.restaurantes.map(valores => valores.horarios);

    //En este arreglo vamos a poner todos los horarios, uno por uno
    var horarios = [];
    arrayHorariosTemp.forEach(function (elemento) {
        elemento.forEach(function (hor) {
            horarios.push(hor)
        });
    });

    //En este arreglo vamos a poner todos los horarios pero sin repetidos
    return elementosFiltrados(horarios);
}
